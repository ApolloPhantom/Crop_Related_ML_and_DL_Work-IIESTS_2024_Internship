...
class CropRecommendationModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        # Encoder
        self.encoder_fc1 = nn.Linear(input_size, 128)
        self.encoder_fc2 = nn.Linear(128, 64)
        self.encoder_fc3 = nn.Linear(64, 32)

        # Decoder
        self.decoder_fc1 = nn.Linear(32, 64)
        self.decoder_fc2 = nn.Linear(64, 128)
        self.decoder_fc3 = nn.Linear(128, input_size)

        # Attention mechanism
        self.attention_fc = nn.Linear(32, 1)
    ...
    def forward(self, x):
        # Encoding
        encoded = self.relu(self.encoder_fc1(x))
        encoded = self.relu(self.encoder_fc2(encoded))
        encoded = self.relu(self.encoder_fc3(encoded))

        # Attention mechanism
        attention_weights = self.sigmoid(self.attention_fc(encoded))
        attended = encoded * attention_weights

        # Decoding (optional, if you want to use autoencoder for reconstruction loss)
        decoded = self.relu(self.decoder_fc1(encoded))
        decoded = self.relu(self.decoder_fc2(decoded))
        decoded = self.sigmoid(self.decoder_fc3(decoded))
