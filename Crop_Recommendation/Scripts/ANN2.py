...
# # Assuming df is your DataFrame with a 'Crop' column
# crop_counts = df['Crop'].value_counts()
# crop_percentages = (crop_counts / crop_counts.sum()) * 100

# plt.figure(figsize=(12, 8))
# ax = sns.barplot(x=crop_counts.index, y=crop_counts.values, palette='dark',hue=crop_counts.index)

# for i, p in enumerate(ax.patches):
#     percentage = crop_percentages[i]
#     ax.annotate(f'{percentage:.3f}%', 
#                 (p.get_x() + p.get_width() / 2., p.get_height()), 
#                 ha='center', va='bottom', 
#                 xytext=(0, 5),  # Adjusting the vertical position
#                 textcoords='offset points', 
#                 rotation=90, 
#                 color='black', 
#                 weight='bold')

# plt.title('Class Distribution of Crop (Percentage)', fontsize=16)
...
# # Define a function to create percentage labels with more precision
# def autopct_func(pct):
#     return f'{pct:.3f}%'

# # Get a color palette from Seaborn
# palette = sns.color_palette("dark", len(crop_counts))

# plt.figure(figsize=(14, 10))
# # Create the pie chart with adjusted label distances and Seaborn palette
# wedges, texts, autotexts = plt.pie(crop_percentages, labels=crop_counts.index, autopct=autopct_func,
#                                    startangle=90, counterclock=False, colors=palette,
#                                    pctdistance=0.85, labeldistance=1.1)

# # Adjust the text properties to ensure readability
