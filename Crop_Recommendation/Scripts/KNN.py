...
def plot_tsne_pca(features, targets, target_names, name, sample_size=100):
...
def train_validation(train_features, test_features, train_target, test_target, name, num_neighbors=5):
    ...
    train_predicted = knn.predict(train_features)
    test_predicted = knn.predict(test_features)
    ...
    all_metrics = []
    fold_metrics = compute_metrics(test_target, test_predicted, knn.classes_.size)
    ...
    for i in range(len(knn.classes_)):
        fpr, tpr, _ = roc_curve(val_target_onehot[:, i], knn.predict_proba(test_features)[:, i])
